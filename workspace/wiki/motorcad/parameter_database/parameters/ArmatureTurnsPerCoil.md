---
type: motorcad_parameter
parameter_name: ArmatureTurnsPerCoil
category: Magnetics
input_output: o/p
data_type: double
units: N/A
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- ArmatureTurnsPerCoil
- Armature Turns Per Coil
- armature_turns_per_coil
- armatureTurnsPerCoil
- armatureturnspercoil
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/ArmatureCopperFreqCompLoss|ArmatureCopperFreqCompLoss]]'
- '[[motorcad/parameter_database/parameters/ArmatureTimeConstant|ArmatureTimeConstant]]'
- '[[motorcad/parameter_database/parameters/ArmatureConductorCSA|ArmatureConductorCSA]]'
- '[[motorcad/parameter_database/parameters/ArmatureTurnCSA|ArmatureTurnCSA]]'
- '[[motorcad/parameter_database/parameters/ArmatureConductorCurrentDensity|ArmatureConductorCurrentDensity]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# ArmatureTurnsPerCoil

## Summary
The average number of turns per coil

## Description
The average number of turns per coil

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: N/A
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `ArmatureTurnsPerCoil`
- `Armature Turns Per Coil`
- `armature_turns_per_coil`
- `armatureTurnsPerCoil`
- `armatureturnspercoil`
- `Magnetics`
- `double`
- `the`
- `average`
- `number`
- `turns`
- `per`
- `coil`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/ArmatureCopperFreqCompLoss|ArmatureCopperFreqCompLoss]]
- [[motorcad/parameter_database/parameters/ArmatureTimeConstant|ArmatureTimeConstant]]
- [[motorcad/parameter_database/parameters/ArmatureConductorCSA|ArmatureConductorCSA]]
- [[motorcad/parameter_database/parameters/ArmatureTurnCSA|ArmatureTurnCSA]]
- [[motorcad/parameter_database/parameters/ArmatureConductorCurrentDensity|ArmatureConductorCurrentDensity]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
