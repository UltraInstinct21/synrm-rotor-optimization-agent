---
type: motorcad_parameter
parameter_name: CS_Area_Rotor_Cage_Bottom
category: Magnetics
input_output: o/p
data_type: double
units: mm²
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- CS_Area_Rotor_Cage_Bottom
- CS Area Rotor Cage Bottom
- cs_area_rotor_cage_bottom
- csAreaRotorCageBottom
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/CS_Area_Rotor_Cage_Top|CS_Area_Rotor_Cage_Top]]'
- '[[motorcad/parameter_database/parameters/CS_Area_Rotor_Cage_Top_Opening|CS_Area_Rotor_Cage_Top_Opening]]'
- '[[motorcad/parameter_database/parameters/CS_Area_Rotor_Cage_Bottom_Opening|CS_Area_Rotor_Cage_Bottom_Opening]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# CS_Area_Rotor_Cage_Bottom

## Summary
Cross sectional area of a Rotor Bar top section

## Description
Cross sectional area of a Rotor Bar top section

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: mm²
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `CS_Area_Rotor_Cage_Bottom`
- `CS Area Rotor Cage Bottom`
- `cs_area_rotor_cage_bottom`
- `csAreaRotorCageBottom`
- `Magnetics`
- `double`
- `cross`
- `sectional`
- `area`
- `rotor`
- `bar`
- `top`
- `section`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/CS_Area_Rotor_Cage_Top|CS_Area_Rotor_Cage_Top]]
- [[motorcad/parameter_database/parameters/CS_Area_Rotor_Cage_Top_Opening|CS_Area_Rotor_Cage_Top_Opening]]
- [[motorcad/parameter_database/parameters/CS_Area_Rotor_Cage_Bottom_Opening|CS_Area_Rotor_Cage_Bottom_Opening]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
