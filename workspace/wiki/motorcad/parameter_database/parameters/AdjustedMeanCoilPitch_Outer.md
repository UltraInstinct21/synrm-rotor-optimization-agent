---
type: motorcad_parameter
parameter_name: AdjustedMeanCoilPitch_Outer
category: Magnetics
input_output: o/p
data_type: double
units: mm
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- AdjustedMeanCoilPitch_Outer
- Adjusted Mean Coil Pitch Outer
- adjusted_mean_coil_pitch_outer
- adjustedMeanCoilPitchOuter
- adjustedmeancoilpitch_outer
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/AdjustedMeanCoilPitch_Aux|AdjustedMeanCoilPitch_Aux]]'
- '[[motorcad/parameter_database/parameters/AdjustedMeanCoilPitch_Inner|AdjustedMeanCoilPitch_Inner]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# AdjustedMeanCoilPitch_Outer

## Summary
The length of average endwinding pitch on the outer side of the stator adjusted to take into account the endwinding multiplier

## Description
The length of average endwinding pitch on the outer side of the stator adjusted to take into account the endwinding multiplier

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: mm
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `AdjustedMeanCoilPitch_Outer`
- `Adjusted Mean Coil Pitch Outer`
- `adjusted_mean_coil_pitch_outer`
- `adjustedMeanCoilPitchOuter`
- `adjustedmeancoilpitch_outer`
- `Magnetics`
- `double`
- `the`
- `length`
- `average`
- `endwinding`
- `pitch`
- `outer`
- `side`
- `stator`
- `adjusted`
- `take`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/AdjustedMeanCoilPitch_Aux|AdjustedMeanCoilPitch_Aux]]
- [[motorcad/parameter_database/parameters/AdjustedMeanCoilPitch_Inner|AdjustedMeanCoilPitch_Inner]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
