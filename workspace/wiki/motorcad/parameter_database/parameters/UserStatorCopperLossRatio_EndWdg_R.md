---
type: motorcad_parameter
parameter_name: UserStatorCopperLossRatio_EndWdg_R
category: Calc_Options
input_output: i/p
data_type: double
units: N/A
default_value: '0.25'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- UserStatorCopperLossRatio_EndWdg_R
- User Stator Copper Loss Ratio End Wdg R
- user_stator_copper_loss_ratio_end_wdg_r
- userStatorCopperLossRatioEndWdgR
- userstatorcopperlossratio_endwdg_r
related_parameters:
- '[[motorcad/parameter_database/parameters/Full_Winding_Circuit_View|Full_Winding_Circuit_View]]'
- '[[motorcad/parameter_database/parameters/TransientGraphUpdateDisabled|TransientGraphUpdateDisabled]]'
- '[[motorcad/parameter_database/parameters/EditingCircuitComponentPosition|EditingCircuitComponentPosition]]'
- '[[motorcad/parameter_database/parameters/CircuitEditing|CircuitEditing]]'
- '[[motorcad/parameter_database/parameters/CircuitFlowEditing|CircuitFlowEditing]]'
- '[[motorcad/parameter_database/parameters/UserStatorCopperLossRatio_Active|UserStatorCopperLossRatio_Active]]'
- '[[motorcad/parameter_database/parameters/UserStatorCopperLossRatio_EndWdg_F|UserStatorCopperLossRatio_EndWdg_F]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# UserStatorCopperLossRatio_EndWdg_R

## Summary
This is the proportion of stator copper losses that are on the rear end winding

## Description
This is the proportion of stator copper losses that are on the rear end winding

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Calc_Options|Calc_Options]]
- **Units**: N/A
- **Default Value**: 0.25
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `UserStatorCopperLossRatio_EndWdg_R`
- `User Stator Copper Loss Ratio End Wdg R`
- `user_stator_copper_loss_ratio_end_wdg_r`
- `userStatorCopperLossRatioEndWdgR`
- `userstatorcopperlossratio_endwdg_r`
- `Calc_Options`
- `double`
- `this`
- `the`
- `proportion`
- `stator`
- `copper`
- `losses`
- `that`
- `are`
- `rear`
- `end`

## Related Parameters
- [[motorcad/parameter_database/parameters/Full_Winding_Circuit_View|Full_Winding_Circuit_View]]
- [[motorcad/parameter_database/parameters/TransientGraphUpdateDisabled|TransientGraphUpdateDisabled]]
- [[motorcad/parameter_database/parameters/EditingCircuitComponentPosition|EditingCircuitComponentPosition]]
- [[motorcad/parameter_database/parameters/CircuitEditing|CircuitEditing]]
- [[motorcad/parameter_database/parameters/CircuitFlowEditing|CircuitFlowEditing]]
- [[motorcad/parameter_database/parameters/UserStatorCopperLossRatio_Active|UserStatorCopperLossRatio_Active]]
- [[motorcad/parameter_database/parameters/UserStatorCopperLossRatio_EndWdg_F|UserStatorCopperLossRatio_EndWdg_F]]

## Related Wiki Pages
- [[equations/loss-equations|Loss Equations]]

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
