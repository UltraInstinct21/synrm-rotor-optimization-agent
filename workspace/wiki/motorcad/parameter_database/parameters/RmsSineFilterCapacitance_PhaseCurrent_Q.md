---
type: motorcad_parameter
parameter_name: RmsSineFilterCapacitance_PhaseCurrent_Q
category: Magnetics
input_output: o/p
data_type: double
units: Amps
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- RmsSineFilterCapacitance_PhaseCurrent_Q
- Rms Sine Filter Capacitance Phase Current Q
- rms_sine_filter_capacitance_phase_current_q
- rmsSineFilterCapacitancePhaseCurrentQ
- rmssinefiltercapacitance_phasecurrent_q
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/RmsSineFilterCapacitance_PhaseCurrent|RmsSineFilterCapacitance_PhaseCurrent]]'
- '[[motorcad/parameter_database/parameters/RmsSineFilterCapacitance_PhaseCurrent_D|RmsSineFilterCapacitance_PhaseCurrent_D]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# RmsSineFilterCapacitance_PhaseCurrent_Q

## Summary
The rms current through the sine filter capacitance in Q axis

## Description
The rms current through the sine filter capacitance in Q axis

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Amps
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `RmsSineFilterCapacitance_PhaseCurrent_Q`
- `Rms Sine Filter Capacitance Phase Current Q`
- `rms_sine_filter_capacitance_phase_current_q`
- `rmsSineFilterCapacitancePhaseCurrentQ`
- `rmssinefiltercapacitance_phasecurrent_q`
- `Magnetics`
- `double`
- `the`
- `rms`
- `current`
- `through`
- `sine`
- `filter`
- `capacitance`
- `axis`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/RmsSineFilterCapacitance_PhaseCurrent|RmsSineFilterCapacitance_PhaseCurrent]]
- [[motorcad/parameter_database/parameters/RmsSineFilterCapacitance_PhaseCurrent_D|RmsSineFilterCapacitance_PhaseCurrent_D]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
