---
type: motorcad_parameter
parameter_name: FieldCurrentDefinition
category: Magnetics
input_output: i/p
data_type: integer
units: N/A
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- FieldCurrentDefinition
- Field Current Definition
- field_current_definition
- fieldCurrentDefinition
- fieldcurrentdefinition
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/FieldCurrentDensity|FieldCurrentDensity]]'
- '[[motorcad/parameter_database/parameters/FieldWindingInductance|FieldWindingInductance]]'
- '[[motorcad/parameter_database/parameters/FieldWindingReactance|FieldWindingReactance]]'
- '[[motorcad/parameter_database/parameters/FieldConductorCSA|FieldConductorCSA]]'
- '[[motorcad/parameter_database/parameters/FieldMLT|FieldMLT]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# FieldCurrentDefinition

## Summary
Defines whether the field current is DC current or current density

## Description
Defines whether the field current is DC current or current density

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: N/A
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `FieldCurrentDefinition`
- `Field Current Definition`
- `field_current_definition`
- `fieldCurrentDefinition`
- `fieldcurrentdefinition`
- `Magnetics`
- `integer`
- `defines`
- `whether`
- `the`
- `field`
- `current`
- `density`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/FieldCurrentDensity|FieldCurrentDensity]]
- [[motorcad/parameter_database/parameters/FieldWindingInductance|FieldWindingInductance]]
- [[motorcad/parameter_database/parameters/FieldWindingReactance|FieldWindingReactance]]
- [[motorcad/parameter_database/parameters/FieldConductorCSA|FieldConductorCSA]]
- [[motorcad/parameter_database/parameters/FieldMLT|FieldMLT]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
