---
type: motorcad_parameter
parameter_name: LastIMSupplyDefinition_SaturationCalc_Backup
category: Magnetics
input_output: i/p
data_type: integer
units: N/A
default_value: '-1'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- LastIMSupplyDefinition_SaturationCalc_Backup
- Last IM Supply Definition Saturation Calc Backup
- last_im_supply_definition_saturation_calc_backup
- lastImSupplyDefinitionSaturationCalcBackup
- lastimsupplydefinition_saturationcalc_backup
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LastIMSupplyDefinition_SaturationCalc|LastIMSupplyDefinition_SaturationCalc]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# LastIMSupplyDefinition_SaturationCalc_Backup

## Summary
The supply definition used in the most recent user-initiated saturation model calculation

## Description
The supply definition used in the most recent user-initiated saturation model calculation

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: N/A
- **Default Value**: -1
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `LastIMSupplyDefinition_SaturationCalc_Backup`
- `Last IM Supply Definition Saturation Calc Backup`
- `last_im_supply_definition_saturation_calc_backup`
- `lastImSupplyDefinitionSaturationCalcBackup`
- `lastimsupplydefinition_saturationcalc_backup`
- `Magnetics`
- `integer`
- `the`
- `supply`
- `definition`
- `used`
- `most`
- `recent`
- `user`
- `initiated`
- `saturation`
- `model`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/LastIMSupplyDefinition_SaturationCalc|LastIMSupplyDefinition_SaturationCalc]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
