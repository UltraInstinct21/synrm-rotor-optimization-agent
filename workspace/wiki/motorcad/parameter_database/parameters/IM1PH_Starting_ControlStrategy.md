---
type: motorcad_parameter
parameter_name: IM1PH_Starting_ControlStrategy
category: Magnetics
input_output: i/p
data_type: integer
units: N/A
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- IM1PH_Starting_ControlStrategy
- IM1 PH Starting Control Strategy
- im1_ph_starting_control_strategy
- im1PhStartingControlStrategy
- im1ph_starting_controlstrategy
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/IM1PH_Starting_CutoffSpeed|IM1PH_Starting_CutoffSpeed]]'
- '[[motorcad/parameter_database/parameters/IM1PH_Starting_CutoffSlip|IM1PH_Starting_CutoffSlip]]'
- '[[motorcad/parameter_database/parameters/IM1PH_Motor_Type|IM1PH_Motor_Type]]'
- '[[motorcad/parameter_database/parameters/IM1PH_Start_Capacitance|IM1PH_Start_Capacitance]]'
- '[[motorcad/parameter_database/parameters/IM1PH_Start_Reactance|IM1PH_Start_Reactance]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# IM1PH_Starting_ControlStrategy

## Summary
Control strategy used to switch from start to run impedances

## Description
Control strategy used to switch from start to run impedances

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: N/A
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `IM1PH_Starting_ControlStrategy`
- `IM1 PH Starting Control Strategy`
- `im1_ph_starting_control_strategy`
- `im1PhStartingControlStrategy`
- `im1ph_starting_controlstrategy`
- `Magnetics`
- `integer`
- `control`
- `strategy`
- `used`
- `switch`
- `from`
- `start`
- `run`
- `impedances`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/IM1PH_Starting_CutoffSpeed|IM1PH_Starting_CutoffSpeed]]
- [[motorcad/parameter_database/parameters/IM1PH_Starting_CutoffSlip|IM1PH_Starting_CutoffSlip]]
- [[motorcad/parameter_database/parameters/IM1PH_Motor_Type|IM1PH_Motor_Type]]
- [[motorcad/parameter_database/parameters/IM1PH_Start_Capacitance|IM1PH_Start_Capacitance]]
- [[motorcad/parameter_database/parameters/IM1PH_Start_Reactance|IM1PH_Start_Reactance]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
