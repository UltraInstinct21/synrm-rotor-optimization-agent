---
type: motorcad_parameter
parameter_name: ArmatureConductor_Resistivity
category: Magnetics
input_output: o/p
data_type: double
units: Ohm.m
default_value: '1.724E-8'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- ArmatureConductor_Resistivity
- Armature Conductor Resistivity
- armature_conductor_resistivity
- armatureConductorResistivity
- armatureconductor_resistivity
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/ArmatureConductor_Temperature|ArmatureConductor_Temperature]]'
- '[[motorcad/parameter_database/parameters/ArmatureConductor_ResistivityAt20C|ArmatureConductor_ResistivityAt20C]]'
- '[[motorcad/parameter_database/parameters/ArmatureConductor_Resistivity_Aux|ArmatureConductor_Resistivity_Aux]]'
- '[[motorcad/parameter_database/parameters/ArmatureConductor_ResistivityAt20C_Aux|ArmatureConductor_ResistivityAt20C_Aux]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# ArmatureConductor_Resistivity

## Summary
The electrical resistivity for the armature conductors at armature conductor temperature

## Description
The electrical resistivity for the armature conductors at armature conductor temperature

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Ohm.m
- **Default Value**: 1.724E-8
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `ArmatureConductor_Resistivity`
- `Armature Conductor Resistivity`
- `armature_conductor_resistivity`
- `armatureConductorResistivity`
- `armatureconductor_resistivity`
- `Magnetics`
- `double`
- `the`
- `electrical`
- `resistivity`
- `for`
- `armature`
- `conductors`
- `conductor`
- `temperature`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/ArmatureConductor_Temperature|ArmatureConductor_Temperature]]
- [[motorcad/parameter_database/parameters/ArmatureConductor_ResistivityAt20C|ArmatureConductor_ResistivityAt20C]]
- [[motorcad/parameter_database/parameters/ArmatureConductor_Resistivity_Aux|ArmatureConductor_Resistivity_Aux]]
- [[motorcad/parameter_database/parameters/ArmatureConductor_ResistivityAt20C_Aux|ArmatureConductor_ResistivityAt20C_Aux]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
