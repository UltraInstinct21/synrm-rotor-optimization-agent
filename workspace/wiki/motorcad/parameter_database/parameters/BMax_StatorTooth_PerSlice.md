---
type: motorcad_parameter
parameter_name: BMax_StatorTooth_PerSlice
category: Magnetics
input_output: o/p
data_type: double
units: Tesla
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- BMax_StatorTooth_PerSlice
- B Max Stator Tooth Per Slice
- b_max_stator_tooth_per_slice
- bMaxStatorToothPerSlice
- bmax_statortooth_perslice
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/BMax_Airgap|BMax_Airgap]]'
- '[[motorcad/parameter_database/parameters/BMax_StatorTooth|BMax_StatorTooth]]'
- '[[motorcad/parameter_database/parameters/BMax_RotorTooth|BMax_RotorTooth]]'
- '[[motorcad/parameter_database/parameters/BMax_StatorBackIron|BMax_StatorBackIron]]'
- '[[motorcad/parameter_database/parameters/BMax_StatorBackIron_PerSlice|BMax_StatorBackIron_PerSlice]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# BMax_StatorTooth_PerSlice

## Summary
Maximum flux density at midpoint in the stator tooth per slice

## Description
Maximum flux density at midpoint in the stator tooth per slice

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Tesla
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `BMax_StatorTooth_PerSlice`
- `B Max Stator Tooth Per Slice`
- `b_max_stator_tooth_per_slice`
- `bMaxStatorToothPerSlice`
- `bmax_statortooth_perslice`
- `Magnetics`
- `double`
- `maximum`
- `flux`
- `density`
- `midpoint`
- `the`
- `stator`
- `tooth`
- `per`
- `slice`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/BMax_Airgap|BMax_Airgap]]
- [[motorcad/parameter_database/parameters/BMax_StatorTooth|BMax_StatorTooth]]
- [[motorcad/parameter_database/parameters/BMax_RotorTooth|BMax_RotorTooth]]
- [[motorcad/parameter_database/parameters/BMax_StatorBackIron|BMax_StatorBackIron]]
- [[motorcad/parameter_database/parameters/BMax_StatorBackIron_PerSlice|BMax_StatorBackIron_PerSlice]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
