---
type: motorcad_parameter
parameter_name: Banding_ResistivityAt20C
category: Magnetics
input_output: i/p
data_type: double
units: Ohm.m
default_value: '1.724E-8'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- Banding_ResistivityAt20C
- Banding Resistivity At20 C
- banding_resistivity_at20_c
- bandingResistivityAt20C
- banding_resistivityat20c
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/Banding_Thickness|Banding_Thickness]]'
- '[[motorcad/parameter_database/parameters/Banding_-_Rotor|Banding_-_Rotor]]'
- '[[motorcad/parameter_database/parameters/Banding_-_Stator|Banding_-_Stator]]'
- '[[motorcad/parameter_database/parameters/Banding_Temperature|Banding_Temperature]]'
- '[[motorcad/parameter_database/parameters/Banding_Resistivity|Banding_Resistivity]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# Banding_ResistivityAt20C

## Summary
The electrical resistivity for the Banding at reference temperature

## Description
The electrical resistivity for the Banding at reference temperature

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Ohm.m
- **Default Value**: 1.724E-8
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `Banding_ResistivityAt20C`
- `Banding Resistivity At20 C`
- `banding_resistivity_at20_c`
- `bandingResistivityAt20C`
- `banding_resistivityat20c`
- `Magnetics`
- `double`
- `the`
- `electrical`
- `resistivity`
- `for`
- `banding`
- `reference`
- `temperature`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/Banding_Thickness|Banding_Thickness]]
- [[motorcad/parameter_database/parameters/Banding_-_Rotor|Banding_-_Rotor]]
- [[motorcad/parameter_database/parameters/Banding_-_Stator|Banding_-_Stator]]
- [[motorcad/parameter_database/parameters/Banding_Temperature|Banding_Temperature]]
- [[motorcad/parameter_database/parameters/Banding_Resistivity|Banding_Resistivity]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
