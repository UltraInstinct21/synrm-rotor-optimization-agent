---
type: motorcad_parameter
parameter_name: AWG_WireGaugeIndex_Litz
category: Winding_Design
input_output: i/p
data_type: integer
units: N/A
default_value: '56'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- AWG_WireGaugeIndex_Litz
- AWG Wire Gauge Index Litz
- awg_wire_gauge_index_litz
- awgWireGaugeIndexLitz
- awg_wiregaugeindex_litz
related_parameters:
- '[[motorcad/parameter_database/parameters/NoMushConductorsDrawn|NoMushConductorsDrawn]]'
- '[[motorcad/parameter_database/parameters/ConductorsSlotBase|ConductorsSlotBase]]'
- '[[motorcad/parameter_database/parameters/ConductorsSlotTooth|ConductorsSlotTooth]]'
- '[[motorcad/parameter_database/parameters/ConductorsVertical|ConductorsVertical]]'
- '[[motorcad/parameter_database/parameters/ConductorsHorizontal|ConductorsHorizontal]]'
- '[[motorcad/parameter_database/parameters/AWG_WireGaugeIndex|AWG_WireGaugeIndex]]'
- '[[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_2|AWG_WireGaugeIndex_2]]'
- '[[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_3|AWG_WireGaugeIndex_3]]'
- '[[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_Rotor|AWG_WireGaugeIndex_Rotor]]'
- '[[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_Aux|AWG_WireGaugeIndex_Aux]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# AWG_WireGaugeIndex_Litz

## Summary
The index for the AWG Litz wire gauge selection

## Description
The index for the AWG Litz wire gauge selection

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Winding_Design|Winding_Design]]
- **Units**: N/A
- **Default Value**: 56
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `AWG_WireGaugeIndex_Litz`
- `AWG Wire Gauge Index Litz`
- `awg_wire_gauge_index_litz`
- `awgWireGaugeIndexLitz`
- `awg_wiregaugeindex_litz`
- `Winding_Design`
- `integer`
- `the`
- `index`
- `for`
- `awg`
- `litz`
- `wire`
- `gauge`
- `selection`

## Related Parameters
- [[motorcad/parameter_database/parameters/NoMushConductorsDrawn|NoMushConductorsDrawn]]
- [[motorcad/parameter_database/parameters/ConductorsSlotBase|ConductorsSlotBase]]
- [[motorcad/parameter_database/parameters/ConductorsSlotTooth|ConductorsSlotTooth]]
- [[motorcad/parameter_database/parameters/ConductorsVertical|ConductorsVertical]]
- [[motorcad/parameter_database/parameters/ConductorsHorizontal|ConductorsHorizontal]]
- [[motorcad/parameter_database/parameters/AWG_WireGaugeIndex|AWG_WireGaugeIndex]]
- [[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_2|AWG_WireGaugeIndex_2]]
- [[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_3|AWG_WireGaugeIndex_3]]
- [[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_Rotor|AWG_WireGaugeIndex_Rotor]]
- [[motorcad/parameter_database/parameters/AWG_WireGaugeIndex_Aux|AWG_WireGaugeIndex_Aux]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
