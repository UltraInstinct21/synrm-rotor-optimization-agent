---
type: motorcad_parameter
parameter_name: FieldConductor_Resistivity
category: Magnetics
input_output: o/p
data_type: double
units: Ohm.m
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- FieldConductor_Resistivity
- Field Conductor Resistivity
- field_conductor_resistivity
- fieldConductorResistivity
- fieldconductor_resistivity
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/FieldConductor_Temperature|FieldConductor_Temperature]]'
- '[[motorcad/parameter_database/parameters/FieldConductor_ResistivityAt20C|FieldConductor_ResistivityAt20C]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# FieldConductor_Resistivity

## Summary
The electrical resistivity for the field conductors at field conductor temperature

## Description
The electrical resistivity for the field conductors at field conductor temperature

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Ohm.m
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `FieldConductor_Resistivity`
- `Field Conductor Resistivity`
- `field_conductor_resistivity`
- `fieldConductorResistivity`
- `fieldconductor_resistivity`
- `Magnetics`
- `double`
- `the`
- `electrical`
- `resistivity`
- `for`
- `field`
- `conductors`
- `conductor`
- `temperature`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/FieldConductor_Temperature|FieldConductor_Temperature]]
- [[motorcad/parameter_database/parameters/FieldConductor_ResistivityAt20C|FieldConductor_ResistivityAt20C]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
