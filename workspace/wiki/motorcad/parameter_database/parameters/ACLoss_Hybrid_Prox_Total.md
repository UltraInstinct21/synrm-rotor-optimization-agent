---
type: motorcad_parameter
parameter_name: ACLoss_Hybrid_Prox_Total
category: Magnetics
input_output: o/p
data_type: double
units: Watts
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- ACLoss_Hybrid_Prox_Total
- AC Loss Hybrid Prox Total
- ac_loss_hybrid_prox_total
- acLossHybridProxTotal
- acloss_hybrid_prox_total
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/ACLoss_FEA_Turns|ACLoss_FEA_Turns]]'
- '[[motorcad/parameter_database/parameters/ACLoss_FEA_OC_PerTurn|ACLoss_FEA_OC_PerTurn]]'
- '[[motorcad/parameter_database/parameters/ACLoss_FEA_OC_Total|ACLoss_FEA_OC_Total]]'
- '[[motorcad/parameter_database/parameters/ACLoss_FEA_OnLoad_PerTurn|ACLoss_FEA_OnLoad_PerTurn]]'
- '[[motorcad/parameter_database/parameters/ACLoss_FEA_OnLoad_Total|ACLoss_FEA_OnLoad_Total]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# ACLoss_Hybrid_Prox_Total

## Summary
Total AC proximity losses (Hybrid method)

## Description
Total AC proximity losses (Hybrid method)

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Watts
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `ACLoss_Hybrid_Prox_Total`
- `AC Loss Hybrid Prox Total`
- `ac_loss_hybrid_prox_total`
- `acLossHybridProxTotal`
- `acloss_hybrid_prox_total`
- `Magnetics`
- `double`
- `total`
- `proximity`
- `losses`
- `hybrid`
- `method`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/ACLoss_FEA_Turns|ACLoss_FEA_Turns]]
- [[motorcad/parameter_database/parameters/ACLoss_FEA_OC_PerTurn|ACLoss_FEA_OC_PerTurn]]
- [[motorcad/parameter_database/parameters/ACLoss_FEA_OC_Total|ACLoss_FEA_OC_Total]]
- [[motorcad/parameter_database/parameters/ACLoss_FEA_OnLoad_PerTurn|ACLoss_FEA_OnLoad_PerTurn]]
- [[motorcad/parameter_database/parameters/ACLoss_FEA_OnLoad_Total|ACLoss_FEA_OnLoad_Total]]

## Related Wiki Pages
- [[equations/loss-equations|Loss Equations]]

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
