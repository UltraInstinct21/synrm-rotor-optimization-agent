---
type: motorcad_parameter
parameter_name: DamperBar_Pitch
category: Dimensions
input_output: i/p
data_type: double
units: EDeg
default_value: '90'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- DamperBar_Pitch
- Damper Bar Pitch
- damper_bar_pitch
- damperBarPitch
- damperbar_pitch
related_parameters:
- '[[motorcad/parameter_database/parameters/DamperBar_Number|DamperBar_Number]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Depth|DamperBar_Depth]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Opening|DamperBar_Opening]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Diameter|DamperBar_Diameter]]'
- '[[motorcad/parameter_database/parameters/Damper_Thickness_F|Damper_Thickness_F]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Temperature|DamperBar_Temperature]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Resistivity|DamperBar_Resistivity]]'
- '[[motorcad/parameter_database/parameters/DamperBar_ResistivityAt20|DamperBar_ResistivityAt20]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# DamperBar_Pitch

## Summary
This is the overall Damper Bar pitch per rotor pole

## Description
This is the overall Damper Bar pitch per rotor pole

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Dimensions|Dimensions]]
- **Units**: EDeg
- **Default Value**: 90
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `DamperBar_Pitch`
- `Damper Bar Pitch`
- `damper_bar_pitch`
- `damperBarPitch`
- `damperbar_pitch`
- `Dimensions`
- `double`
- `this`
- `the`
- `overall`
- `damper`
- `bar`
- `pitch`
- `per`
- `rotor`
- `pole`

## Related Parameters
- [[motorcad/parameter_database/parameters/DamperBar_Number|DamperBar_Number]]
- [[motorcad/parameter_database/parameters/DamperBar_Depth|DamperBar_Depth]]
- [[motorcad/parameter_database/parameters/DamperBar_Opening|DamperBar_Opening]]
- [[motorcad/parameter_database/parameters/DamperBar_Diameter|DamperBar_Diameter]]
- [[motorcad/parameter_database/parameters/Damper_Thickness_F|Damper_Thickness_F]]
- [[motorcad/parameter_database/parameters/DamperBar_Temperature|DamperBar_Temperature]]
- [[motorcad/parameter_database/parameters/DamperBar_Resistivity|DamperBar_Resistivity]]
- [[motorcad/parameter_database/parameters/DamperBar_ResistivityAt20|DamperBar_ResistivityAt20]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
