---
type: motorcad_parameter
parameter_name: CustomDriveCyclePointReduction
category: Magnetics
input_output: i/p
data_type: integer
units: N/A
default_value: '1'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- CustomDriveCyclePointReduction
- Custom Drive Cycle Point Reduction
- custom_drive_cycle_point_reduction
- customDriveCyclePointReduction
- customdrivecyclepointreduction
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/CustomOutputType|CustomOutputType]]'
- '[[motorcad/parameter_database/parameters/CustomLoadPointSettings|CustomLoadPointSettings]]'
- '[[motorcad/parameter_database/parameters/CustomLoadPointOutput|CustomLoadPointOutput]]'
- '[[motorcad/parameter_database/parameters/CustomMaterialLossMethod|CustomMaterialLossMethod]]'
- '[[motorcad/parameter_database/parameters/CustomDriveDefinition|CustomDriveDefinition]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# CustomDriveCyclePointReduction

## Summary
The point reduction used for importing custom drive cycles. When set to 1 then all data points are imported from a file.

## Description
The point reduction used for importing custom drive cycles. When set to 1 then all data points are imported from a file.

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: N/A
- **Default Value**: 1
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `CustomDriveCyclePointReduction`
- `Custom Drive Cycle Point Reduction`
- `custom_drive_cycle_point_reduction`
- `customDriveCyclePointReduction`
- `customdrivecyclepointreduction`
- `Magnetics`
- `integer`
- `the`
- `point`
- `reduction`
- `used`
- `for`
- `importing`
- `custom`
- `drive`
- `cycles`
- `when`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/CustomOutputType|CustomOutputType]]
- [[motorcad/parameter_database/parameters/CustomLoadPointSettings|CustomLoadPointSettings]]
- [[motorcad/parameter_database/parameters/CustomLoadPointOutput|CustomLoadPointOutput]]
- [[motorcad/parameter_database/parameters/CustomMaterialLossMethod|CustomMaterialLossMethod]]
- [[motorcad/parameter_database/parameters/CustomDriveDefinition|CustomDriveDefinition]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
