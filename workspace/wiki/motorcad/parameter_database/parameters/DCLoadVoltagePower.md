---
type: motorcad_parameter
parameter_name: DCLoadVoltagePower
category: Magnetics
input_output: o/p
data_type: double
units: Watts
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- DCLoadVoltagePower
- DC Load Voltage Power
- dc_load_voltage_power
- dcLoadVoltagePower
- dcloadvoltagepower
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/DCLoadResistorVoltage|DCLoadResistorVoltage]]'
- '[[motorcad/parameter_database/parameters/DCLoadResistorPower|DCLoadResistorPower]]'
- '[[motorcad/parameter_database/parameters/DCLoadType|DCLoadType]]'
- '[[motorcad/parameter_database/parameters/DCLoadResistance|DCLoadResistance]]'
- '[[motorcad/parameter_database/parameters/DCLoadVoltage|DCLoadVoltage]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# DCLoadVoltagePower

## Summary
The power absorbed by the DC load voltage

## Description
The power absorbed by the DC load voltage

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Watts
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `DCLoadVoltagePower`
- `DC Load Voltage Power`
- `dc_load_voltage_power`
- `dcLoadVoltagePower`
- `dcloadvoltagepower`
- `Magnetics`
- `double`
- `the`
- `power`
- `absorbed`
- `load`
- `voltage`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/DCLoadResistorVoltage|DCLoadResistorVoltage]]
- [[motorcad/parameter_database/parameters/DCLoadResistorPower|DCLoadResistorPower]]
- [[motorcad/parameter_database/parameters/DCLoadType|DCLoadType]]
- [[motorcad/parameter_database/parameters/DCLoadResistance|DCLoadResistance]]
- [[motorcad/parameter_database/parameters/DCLoadVoltage|DCLoadVoltage]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
