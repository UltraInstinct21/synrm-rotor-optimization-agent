---
type: motorcad_parameter
parameter_name: CurrentDefinition
category: Magnetics
input_output: i/p
data_type: integer
units: N/A
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- CurrentDefinition
- Current Definition
- current_definition
- currentDefinition
- currentdefinition
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/CurrentAngle|CurrentAngle]]'
- '[[motorcad/parameter_database/parameters/CurrentGraphValues|CurrentGraphValues]]'
- '[[motorcad/parameter_database/parameters/CurrentTime|CurrentTime]]'
- '[[motorcad/parameter_database/parameters/CurrentConvergenceDamping|CurrentConvergenceDamping]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# CurrentDefinition

## Summary
Defines whether the current input is rms current, peak current or rms current density

## Description
Defines whether the current input is rms current, peak current or rms current density

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: N/A
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `CurrentDefinition`
- `Current Definition`
- `current_definition`
- `currentDefinition`
- `currentdefinition`
- `Magnetics`
- `integer`
- `defines`
- `whether`
- `the`
- `current`
- `input`
- `rms`
- `peak`
- `density`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/CurrentAngle|CurrentAngle]]
- [[motorcad/parameter_database/parameters/CurrentGraphValues|CurrentGraphValues]]
- [[motorcad/parameter_database/parameters/CurrentTime|CurrentTime]]
- [[motorcad/parameter_database/parameters/CurrentConvergenceDamping|CurrentConvergenceDamping]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
