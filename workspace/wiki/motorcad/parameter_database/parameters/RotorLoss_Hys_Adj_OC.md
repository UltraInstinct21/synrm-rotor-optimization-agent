---
type: motorcad_parameter
parameter_name: RotorLoss_Hys_Adj_OC
category: Magnetics
input_output: o/p
data_type: double
units: Watts
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- RotorLoss_Hys_Adj_OC
- Rotor Loss Hys Adj OC
- rotor_loss_hys_adj_oc
- rotorLossHysAdjOc
- rotorloss_hys_adj_oc
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/RotorLoss_Fundamental_Hys|RotorLoss_Fundamental_Hys]]'
- '[[motorcad/parameter_database/parameters/RotorLoss_Minor_Hys|RotorLoss_Minor_Hys]]'
- '[[motorcad/parameter_database/parameters/RotorLoss_Hys|RotorLoss_Hys]]'
- '[[motorcad/parameter_database/parameters/RotorLoss_Hys_Adj|RotorLoss_Hys_Adj]]'
- '[[motorcad/parameter_database/parameters/RotorLoss_Eddy|RotorLoss_Eddy]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# RotorLoss_Hys_Adj_OC

## Summary
Rotor iron losses from hysteresis (adjusted for build factor)

## Description
Rotor iron losses from hysteresis (adjusted for build factor)

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Watts
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `RotorLoss_Hys_Adj_OC`
- `Rotor Loss Hys Adj OC`
- `rotor_loss_hys_adj_oc`
- `rotorLossHysAdjOc`
- `rotorloss_hys_adj_oc`
- `Magnetics`
- `double`
- `rotor`
- `iron`
- `losses`
- `from`
- `hysteresis`
- `adjusted`
- `for`
- `build`
- `factor`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/RotorLoss_Fundamental_Hys|RotorLoss_Fundamental_Hys]]
- [[motorcad/parameter_database/parameters/RotorLoss_Minor_Hys|RotorLoss_Minor_Hys]]
- [[motorcad/parameter_database/parameters/RotorLoss_Hys|RotorLoss_Hys]]
- [[motorcad/parameter_database/parameters/RotorLoss_Hys_Adj|RotorLoss_Hys_Adj]]
- [[motorcad/parameter_database/parameters/RotorLoss_Eddy|RotorLoss_Eddy]]

## Related Wiki Pages
- [[equations/loss-equations|Loss Equations]]

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
