---
type: motorcad_parameter
parameter_name: Fan_AxialPosition
category: Dimensions
input_output: i/p
data_type: double
units: mm
default_value: '188'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- Fan_AxialPosition
- Fan Axial Position
- fan_axial_position
- fanAxialPosition
- fan_axialposition
related_parameters:
- '[[motorcad/parameter_database/parameters/DamperBar_Number|DamperBar_Number]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Depth|DamperBar_Depth]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Opening|DamperBar_Opening]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Diameter|DamperBar_Diameter]]'
- '[[motorcad/parameter_database/parameters/DamperBar_Pitch|DamperBar_Pitch]]'
- '[[motorcad/parameter_database/parameters/Fan_Definition|Fan_Definition]]'
- '[[motorcad/parameter_database/parameters/Fan_Diameter|Fan_Diameter]]'
- '[[motorcad/parameter_database/parameters/Fan_Blade_Axial_Offset|Fan_Blade_Axial_Offset]]'
- '[[motorcad/parameter_database/parameters/Fan_Blade_Width|Fan_Blade_Width]]'
- '[[motorcad/parameter_database/parameters/Fan_BackPlate_Width|Fan_BackPlate_Width]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# Fan_AxialPosition

## Summary
This is the axial position of the shaft mounted fan.

## Description
This is the axial position of the shaft mounted fan.

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Dimensions|Dimensions]]
- **Units**: mm
- **Default Value**: 188
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `Fan_AxialPosition`
- `Fan Axial Position`
- `fan_axial_position`
- `fanAxialPosition`
- `fan_axialposition`
- `Dimensions`
- `double`
- `this`
- `the`
- `axial`
- `position`
- `shaft`
- `mounted`
- `fan`

## Related Parameters
- [[motorcad/parameter_database/parameters/DamperBar_Number|DamperBar_Number]]
- [[motorcad/parameter_database/parameters/DamperBar_Depth|DamperBar_Depth]]
- [[motorcad/parameter_database/parameters/DamperBar_Opening|DamperBar_Opening]]
- [[motorcad/parameter_database/parameters/DamperBar_Diameter|DamperBar_Diameter]]
- [[motorcad/parameter_database/parameters/DamperBar_Pitch|DamperBar_Pitch]]
- [[motorcad/parameter_database/parameters/Fan_Definition|Fan_Definition]]
- [[motorcad/parameter_database/parameters/Fan_Diameter|Fan_Diameter]]
- [[motorcad/parameter_database/parameters/Fan_Blade_Axial_Offset|Fan_Blade_Axial_Offset]]
- [[motorcad/parameter_database/parameters/Fan_Blade_Width|Fan_Blade_Width]]
- [[motorcad/parameter_database/parameters/Fan_BackPlate_Width|Fan_BackPlate_Width]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
