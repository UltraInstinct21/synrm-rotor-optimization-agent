---
type: motorcad_parameter
parameter_name: ConductorCentre_R_x
category: Winding_Design
input_output: i/p
data_type: double
units: mm
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- ConductorCentre_R_x
- Conductor Centre R x
- conductor_centre_r_x
- conductorCentreRX
- conductorcentre_r_x
related_parameters:
- '[[motorcad/parameter_database/parameters/NoMushConductorsDrawn|NoMushConductorsDrawn]]'
- '[[motorcad/parameter_database/parameters/ConductorsSlotBase|ConductorsSlotBase]]'
- '[[motorcad/parameter_database/parameters/ConductorsSlotTooth|ConductorsSlotTooth]]'
- '[[motorcad/parameter_database/parameters/ConductorsVertical|ConductorsVertical]]'
- '[[motorcad/parameter_database/parameters/ConductorsHorizontal|ConductorsHorizontal]]'
- '[[motorcad/parameter_database/parameters/ConductorCentre_L_x|ConductorCentre_L_x]]'
- '[[motorcad/parameter_database/parameters/ConductorCentre_L_y|ConductorCentre_L_y]]'
- '[[motorcad/parameter_database/parameters/ConductorCentre_R_y|ConductorCentre_R_y]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# ConductorCentre_R_x

## Summary
Array of Conductor centre x coordinates at right side of slot

## Description
Array of Conductor centre x coordinates at right side of slot

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Winding_Design|Winding_Design]]
- **Units**: mm
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `ConductorCentre_R_x`
- `Conductor Centre R x`
- `conductor_centre_r_x`
- `conductorCentreRX`
- `conductorcentre_r_x`
- `Winding_Design`
- `double`
- `array`
- `conductor`
- `centre`
- `coordinates`
- `right`
- `side`
- `slot`

## Related Parameters
- [[motorcad/parameter_database/parameters/NoMushConductorsDrawn|NoMushConductorsDrawn]]
- [[motorcad/parameter_database/parameters/ConductorsSlotBase|ConductorsSlotBase]]
- [[motorcad/parameter_database/parameters/ConductorsSlotTooth|ConductorsSlotTooth]]
- [[motorcad/parameter_database/parameters/ConductorsVertical|ConductorsVertical]]
- [[motorcad/parameter_database/parameters/ConductorsHorizontal|ConductorsHorizontal]]
- [[motorcad/parameter_database/parameters/ConductorCentre_L_x|ConductorCentre_L_x]]
- [[motorcad/parameter_database/parameters/ConductorCentre_L_y|ConductorCentre_L_y]]
- [[motorcad/parameter_database/parameters/ConductorCentre_R_y|ConductorCentre_R_y]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
