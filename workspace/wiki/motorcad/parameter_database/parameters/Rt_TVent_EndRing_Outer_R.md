---
type: motorcad_parameter
parameter_name: Rt_TVent_EndRing_Outer_R
category: Through_Vent
input_output: o/p
data_type: double
units: °C/W
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- Rt_TVent_EndRing_Outer_R
- Rt T Vent End Ring Outer R
- rt_t_vent_end_ring_outer_r
- rtTVentEndRingOuterR
- rt_tvent_endring_outer_r
related_parameters:
- '[[motorcad/parameter_database/parameters/TVent_Fan_Definition|TVent_Fan_Definition]]'
- '[[motorcad/parameter_database/parameters/TVent_Shaft_Speed|TVent_Shaft_Speed]]'
- '[[motorcad/parameter_database/parameters/TVent_Flow_Rate|TVent_Flow_Rate]]'
- '[[motorcad/parameter_database/parameters/Flow_Rate_Airgap_Duct_Wall_F|Flow_Rate_Airgap_Duct_Wall_F]]'
- '[[motorcad/parameter_database/parameters/Flow_Rate_Airgap_Duct_Wall_R|Flow_Rate_Airgap_Duct_Wall_R]]'
- '[[motorcad/parameter_database/parameters/Rt_EndW_Axle_Front|Rt_EndW_Axle_Front]]'
- '[[motorcad/parameter_database/parameters/Rt_EndW_Axle_Rear|Rt_EndW_Axle_Rear]]'
- '[[motorcad/parameter_database/parameters/Rt_Airgap_Cup_Endcap_Con|Rt_Airgap_Cup_Endcap_Con]]'
- '[[motorcad/parameter_database/parameters/Rt_Airgap_Cup_Endcap_Rad|Rt_Airgap_Cup_Endcap_Rad]]'
- '[[motorcad/parameter_database/parameters/Rt_ES_EndRing_Bore_F|Rt_ES_EndRing_Bore_F]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# Rt_TVent_EndRing_Outer_R

## Summary
The thermal resistance from the outer surface of rear rotor end ring to the fluid

## Description
The thermal resistance from the outer surface of rear rotor end ring to the fluid

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Through_Vent|Through_Vent]]
- **Units**: °C/W
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `Rt_TVent_EndRing_Outer_R`
- `Rt T Vent End Ring Outer R`
- `rt_t_vent_end_ring_outer_r`
- `rtTVentEndRingOuterR`
- `rt_tvent_endring_outer_r`
- `Through_Vent`
- `double`
- `the`
- `thermal`
- `resistance`
- `from`
- `outer`
- `surface`
- `rear`
- `rotor`
- `end`
- `ring`

## Related Parameters
- [[motorcad/parameter_database/parameters/TVent_Fan_Definition|TVent_Fan_Definition]]
- [[motorcad/parameter_database/parameters/TVent_Shaft_Speed|TVent_Shaft_Speed]]
- [[motorcad/parameter_database/parameters/TVent_Flow_Rate|TVent_Flow_Rate]]
- [[motorcad/parameter_database/parameters/Flow_Rate_Airgap_Duct_Wall_F|Flow_Rate_Airgap_Duct_Wall_F]]
- [[motorcad/parameter_database/parameters/Flow_Rate_Airgap_Duct_Wall_R|Flow_Rate_Airgap_Duct_Wall_R]]
- [[motorcad/parameter_database/parameters/Rt_EndW_Axle_Front|Rt_EndW_Axle_Front]]
- [[motorcad/parameter_database/parameters/Rt_EndW_Axle_Rear|Rt_EndW_Axle_Rear]]
- [[motorcad/parameter_database/parameters/Rt_Airgap_Cup_Endcap_Con|Rt_Airgap_Cup_Endcap_Con]]
- [[motorcad/parameter_database/parameters/Rt_Airgap_Cup_Endcap_Rad|Rt_Airgap_Cup_Endcap_Rad]]
- [[motorcad/parameter_database/parameters/Rt_ES_EndRing_Bore_F|Rt_ES_EndRing_Bore_F]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
