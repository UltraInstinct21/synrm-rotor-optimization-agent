---
type: motorcad_parameter
parameter_name: IMLookup_StatorLeakageInductance
category: Magnetics
input_output: i/p
data_type: double
units: Henry
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- IMLookup_StatorLeakageInductance
- IM Lookup Stator Leakage Inductance
- im_lookup_stator_leakage_inductance
- imLookupStatorLeakageInductance
- imlookup_statorleakageinductance
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/IMLookup_ExtensionFactor|IMLookup_ExtensionFactor]]'
- '[[motorcad/parameter_database/parameters/IMLookup_MaxImagRequired|IMLookup_MaxImagRequired]]'
- '[[motorcad/parameter_database/parameters/IMLookup_LastExtensionFactor|IMLookup_LastExtensionFactor]]'
- '[[motorcad/parameter_database/parameters/IMLookup_Imag|IMLookup_Imag]]'
- '[[motorcad/parameter_database/parameters/IMLookup_SaturationFactor|IMLookup_SaturationFactor]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# IMLookup_StatorLeakageInductance

## Summary
The stator leakage inductance calculated in the IM FEA saturation model

## Description
The stator leakage inductance calculated in the IM FEA saturation model

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Henry
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `IMLookup_StatorLeakageInductance`
- `IM Lookup Stator Leakage Inductance`
- `im_lookup_stator_leakage_inductance`
- `imLookupStatorLeakageInductance`
- `imlookup_statorleakageinductance`
- `Magnetics`
- `double`
- `the`
- `stator`
- `leakage`
- `inductance`
- `calculated`
- `fea`
- `saturation`
- `model`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/IMLookup_ExtensionFactor|IMLookup_ExtensionFactor]]
- [[motorcad/parameter_database/parameters/IMLookup_MaxImagRequired|IMLookup_MaxImagRequired]]
- [[motorcad/parameter_database/parameters/IMLookup_LastExtensionFactor|IMLookup_LastExtensionFactor]]
- [[motorcad/parameter_database/parameters/IMLookup_Imag|IMLookup_Imag]]
- [[motorcad/parameter_database/parameters/IMLookup_SaturationFactor|IMLookup_SaturationFactor]]

## Related Wiki Pages
- [[equations/inductance-equations|Inductance Equations]]
- [[concepts/saliency-ratio|Saliency Ratio]]

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
