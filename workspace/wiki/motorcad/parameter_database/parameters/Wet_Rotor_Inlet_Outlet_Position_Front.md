---
type: motorcad_parameter
parameter_name: Wet_Rotor_Inlet_Outlet_Position_Front
category: Calc_Options
input_output: i/p
data_type: integer
units: N/A
default_value: N/A
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- Wet_Rotor_Inlet_Outlet_Position_Front
- Wet Rotor Inlet Outlet Position Front
- wet_rotor_inlet_outlet_position_front
- wetRotorInletOutletPositionFront
related_parameters:
- '[[motorcad/parameter_database/parameters/Full_Winding_Circuit_View|Full_Winding_Circuit_View]]'
- '[[motorcad/parameter_database/parameters/TransientGraphUpdateDisabled|TransientGraphUpdateDisabled]]'
- '[[motorcad/parameter_database/parameters/EditingCircuitComponentPosition|EditingCircuitComponentPosition]]'
- '[[motorcad/parameter_database/parameters/CircuitEditing|CircuitEditing]]'
- '[[motorcad/parameter_database/parameters/CircuitFlowEditing|CircuitFlowEditing]]'
- '[[motorcad/parameter_database/parameters/Wet_Rotor_Dissipation_Total|Wet_Rotor_Dissipation_Total]]'
- '[[motorcad/parameter_database/parameters/Wet_Rotor_Fluid_Dynamic_Viscosity|Wet_Rotor_Fluid_Dynamic_Viscosity]]'
- '[[motorcad/parameter_database/parameters/Wet_Rotor_Fluid_Pr|Wet_Rotor_Fluid_Pr]]'
- '[[motorcad/parameter_database/parameters/Wet_Rotor_Airgap_Inlet_Temp|Wet_Rotor_Airgap_Inlet_Temp]]'
- '[[motorcad/parameter_database/parameters/Wet_Rotor_Airgap_Outlet_Temp|Wet_Rotor_Airgap_Outlet_Temp]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# Wet_Rotor_Inlet_Outlet_Position_Front

## Summary
Motor-CAD automation parameter `Wet_Rotor_Inlet_Outlet_Position_Front` in category `Calc_Options`.

## Description
No description provided in the source database.

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Calc_Options|Calc_Options]]
- **Units**: N/A
- **Default Value**: N/A
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `Wet_Rotor_Inlet_Outlet_Position_Front`
- `Wet Rotor Inlet Outlet Position Front`
- `wet_rotor_inlet_outlet_position_front`
- `wetRotorInletOutletPositionFront`
- `Calc_Options`
- `integer`

## Related Parameters
- [[motorcad/parameter_database/parameters/Full_Winding_Circuit_View|Full_Winding_Circuit_View]]
- [[motorcad/parameter_database/parameters/TransientGraphUpdateDisabled|TransientGraphUpdateDisabled]]
- [[motorcad/parameter_database/parameters/EditingCircuitComponentPosition|EditingCircuitComponentPosition]]
- [[motorcad/parameter_database/parameters/CircuitEditing|CircuitEditing]]
- [[motorcad/parameter_database/parameters/CircuitFlowEditing|CircuitFlowEditing]]
- [[motorcad/parameter_database/parameters/Wet_Rotor_Dissipation_Total|Wet_Rotor_Dissipation_Total]]
- [[motorcad/parameter_database/parameters/Wet_Rotor_Fluid_Dynamic_Viscosity|Wet_Rotor_Fluid_Dynamic_Viscosity]]
- [[motorcad/parameter_database/parameters/Wet_Rotor_Fluid_Pr|Wet_Rotor_Fluid_Pr]]
- [[motorcad/parameter_database/parameters/Wet_Rotor_Airgap_Inlet_Temp|Wet_Rotor_Airgap_Inlet_Temp]]
- [[motorcad/parameter_database/parameters/Wet_Rotor_Airgap_Outlet_Temp|Wet_Rotor_Airgap_Outlet_Temp]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
