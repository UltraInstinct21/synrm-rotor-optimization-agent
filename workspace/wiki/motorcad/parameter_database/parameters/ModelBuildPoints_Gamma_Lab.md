---
type: motorcad_parameter
parameter_name: ModelBuildPoints_Gamma_Lab
category: ModelParameters_MotorLAB
input_output: i/p
data_type: integer
units: N/A
default_value: '5'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- ModelBuildPoints_Gamma_Lab
- Model Build Points Gamma Lab
- model_build_points_gamma_lab
- modelBuildPointsGammaLab
- modelbuildpoints_gamma_lab
related_parameters:
- '[[motorcad/parameter_database/parameters/PsiQ_coeff_MotorLAB|PsiQ_coeff_MotorLAB]]'
- '[[motorcad/parameter_database/parameters/PsiD_coeff_MotorLAB|PsiD_coeff_MotorLAB]]'
- '[[motorcad/parameter_database/parameters/EndWindingResistance_Lab|EndWindingResistance_Lab]]'
- '[[motorcad/parameter_database/parameters/EndWindingResistance_Field_Lab|EndWindingResistance_Field_Lab]]'
- '[[motorcad/parameter_database/parameters/EndWindingInductance_Lab|EndWindingInductance_Lab]]'
- '[[motorcad/parameter_database/parameters/ModelBuildPoints_ResolutionType_Lab|ModelBuildPoints_ResolutionType_Lab]]'
- '[[motorcad/parameter_database/parameters/ModelBuildPoints_Current_Lab|ModelBuildPoints_Current_Lab]]'
- '[[motorcad/parameter_database/parameters/ModelBuildPoints_RotorCurrent_Lab|ModelBuildPoints_RotorCurrent_Lab]]'
- '[[motorcad/parameter_database/parameters/ModelBuildPoints_Speed_Lab|ModelBuildPoints_Speed_Lab]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# ModelBuildPoints_Gamma_Lab

## Summary
Number of phase advance points in Lab saturation/loss model build

## Description
Number of phase advance points in Lab saturation/loss model build

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/ModelParameters_MotorLAB|ModelParameters_MotorLAB]]
- **Units**: N/A
- **Default Value**: 5
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `ModelBuildPoints_Gamma_Lab`
- `Model Build Points Gamma Lab`
- `model_build_points_gamma_lab`
- `modelBuildPointsGammaLab`
- `modelbuildpoints_gamma_lab`
- `ModelParameters_MotorLAB`
- `integer`
- `number`
- `phase`
- `advance`
- `points`
- `lab`
- `saturation`
- `loss`
- `model`
- `build`

## Related Parameters
- [[motorcad/parameter_database/parameters/PsiQ_coeff_MotorLAB|PsiQ_coeff_MotorLAB]]
- [[motorcad/parameter_database/parameters/PsiD_coeff_MotorLAB|PsiD_coeff_MotorLAB]]
- [[motorcad/parameter_database/parameters/EndWindingResistance_Lab|EndWindingResistance_Lab]]
- [[motorcad/parameter_database/parameters/EndWindingResistance_Field_Lab|EndWindingResistance_Field_Lab]]
- [[motorcad/parameter_database/parameters/EndWindingInductance_Lab|EndWindingInductance_Lab]]
- [[motorcad/parameter_database/parameters/ModelBuildPoints_ResolutionType_Lab|ModelBuildPoints_ResolutionType_Lab]]
- [[motorcad/parameter_database/parameters/ModelBuildPoints_Current_Lab|ModelBuildPoints_Current_Lab]]
- [[motorcad/parameter_database/parameters/ModelBuildPoints_RotorCurrent_Lab|ModelBuildPoints_RotorCurrent_Lab]]
- [[motorcad/parameter_database/parameters/ModelBuildPoints_Speed_Lab|ModelBuildPoints_Speed_Lab]]

## Related Wiki Pages
- [[design_guidelines/current-angle-optimization|Current Angle Optimization]]

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
