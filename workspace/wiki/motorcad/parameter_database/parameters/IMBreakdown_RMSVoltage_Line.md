---
type: motorcad_parameter
parameter_name: IMBreakdown_RMSVoltage_Line
category: Magnetics
input_output: o/p
data_type: double
units: Volts
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- IMBreakdown_RMSVoltage_Line
- IM Breakdown RMS Voltage Line
- im_breakdown_rms_voltage_line
- imBreakdownRmsVoltageLine
- imbreakdown_rmsvoltage_line
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/IMBreakdown_Torque|IMBreakdown_Torque]]'
- '[[motorcad/parameter_database/parameters/IMBreakdown_ShaftTorque|IMBreakdown_ShaftTorque]]'
- '[[motorcad/parameter_database/parameters/IMBreakdown_RMSCurrent_Line|IMBreakdown_RMSCurrent_Line]]'
- '[[motorcad/parameter_database/parameters/IMBreakdown_RMSCurrent_Phase|IMBreakdown_RMSCurrent_Phase]]'
- '[[motorcad/parameter_database/parameters/IMBreakdown_RMSVoltage_Phase|IMBreakdown_RMSVoltage_Phase]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# IMBreakdown_RMSVoltage_Line

## Summary
The breakdown rms line voltage

## Description
The breakdown rms line voltage

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Volts
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `IMBreakdown_RMSVoltage_Line`
- `IM Breakdown RMS Voltage Line`
- `im_breakdown_rms_voltage_line`
- `imBreakdownRmsVoltageLine`
- `imbreakdown_rmsvoltage_line`
- `Magnetics`
- `double`
- `the`
- `breakdown`
- `rms`
- `line`
- `voltage`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/IMBreakdown_Torque|IMBreakdown_Torque]]
- [[motorcad/parameter_database/parameters/IMBreakdown_ShaftTorque|IMBreakdown_ShaftTorque]]
- [[motorcad/parameter_database/parameters/IMBreakdown_RMSCurrent_Line|IMBreakdown_RMSCurrent_Line]]
- [[motorcad/parameter_database/parameters/IMBreakdown_RMSCurrent_Phase|IMBreakdown_RMSCurrent_Phase]]
- [[motorcad/parameter_database/parameters/IMBreakdown_RMSVoltage_Phase|IMBreakdown_RMSVoltage_Phase]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
