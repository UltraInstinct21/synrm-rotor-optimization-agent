---
type: motorcad_parameter
parameter_name: MultiForceMaxEnvelopeSpeed
category: Magnetics
input_output: i/p
data_type: integer
units: rpm
default_value: '10000'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- MultiForceMaxEnvelopeSpeed
- Multi Force Max Envelope Speed
- multi_force_max_envelope_speed
- multiForceMaxEnvelopeSpeed
- multiforcemaxenvelopespeed
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/MultiForceExportFile|MultiForceExportFile]]'
- '[[motorcad/parameter_database/parameters/MultiForceCalcsRunning|MultiForceCalcsRunning]]'
- '[[motorcad/parameter_database/parameters/MultiForceLoadPointDefinition|MultiForceLoadPointDefinition]]'
- '[[motorcad/parameter_database/parameters/MultiForceMinEnvelopeSpeed|MultiForceMinEnvelopeSpeed]]'
- '[[motorcad/parameter_database/parameters/MultiForceEnvelopePoints|MultiForceEnvelopePoints]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# MultiForceMaxEnvelopeSpeed

## Summary
The Maximum speed used in calculating the torque speed envelope for multi-point force calculations

## Description
The Maximum speed used in calculating the torque speed envelope for multi-point force calculations

## Metadata
- **Input/Output**: i/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: rpm
- **Default Value**: 10000
- **Data Type**: [[motorcad/parameter_database/data_types/integer|integer]]

## Search Keywords
- `MultiForceMaxEnvelopeSpeed`
- `Multi Force Max Envelope Speed`
- `multi_force_max_envelope_speed`
- `multiForceMaxEnvelopeSpeed`
- `multiforcemaxenvelopespeed`
- `Magnetics`
- `integer`
- `the`
- `maximum`
- `speed`
- `used`
- `calculating`
- `torque`
- `envelope`
- `for`
- `multi`
- `point`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/MultiForceExportFile|MultiForceExportFile]]
- [[motorcad/parameter_database/parameters/MultiForceCalcsRunning|MultiForceCalcsRunning]]
- [[motorcad/parameter_database/parameters/MultiForceLoadPointDefinition|MultiForceLoadPointDefinition]]
- [[motorcad/parameter_database/parameters/MultiForceMinEnvelopeSpeed|MultiForceMinEnvelopeSpeed]]
- [[motorcad/parameter_database/parameters/MultiForceEnvelopePoints|MultiForceEnvelopePoints]]

## Related Wiki Pages
No direct engineering concept page linked.

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
