---
type: motorcad_parameter
parameter_name: HousingLoss_Fundamental_Hys
category: Magnetics
input_output: o/p
data_type: double
units: Watts
default_value: '0'
source_file: D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx
aliases:
- HousingLoss_Fundamental_Hys
- Housing Loss Fundamental Hys
- housing_loss_fundamental_hys
- housingLossFundamentalHys
- housingloss_fundamental_hys
related_parameters:
- '[[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]'
- '[[motorcad/parameter_database/parameters/HousingLoss_Hys_OC|HousingLoss_Hys_OC]]'
- '[[motorcad/parameter_database/parameters/HousingLoss_Hys_Adj_OC|HousingLoss_Hys_Adj_OC]]'
- '[[motorcad/parameter_database/parameters/HousingLoss_Fundamental_Hys_OC|HousingLoss_Fundamental_Hys_OC]]'
- '[[motorcad/parameter_database/parameters/HousingLoss_Minor_Hys_OC|HousingLoss_Minor_Hys_OC]]'
- '[[motorcad/parameter_database/parameters/HousingLoss_Total_Adj_OC|HousingLoss_Total_Adj_OC]]'
related_workflows: []
related_concepts: []
confidence: Verified
---

# HousingLoss_Fundamental_Hys

## Summary
Housing losses from hysteresis

## Description
Housing losses from hysteresis

## Metadata
- **Input/Output**: o/p
- **Category**: [[motorcad/parameter_database/categories/Magnetics|Magnetics]]
- **Units**: Watts
- **Default Value**: 0
- **Data Type**: [[motorcad/parameter_database/data_types/double|double]]

## Search Keywords
- `HousingLoss_Fundamental_Hys`
- `Housing Loss Fundamental Hys`
- `housing_loss_fundamental_hys`
- `housingLossFundamentalHys`
- `housingloss_fundamental_hys`
- `Magnetics`
- `double`
- `housing`
- `losses`
- `from`
- `hysteresis`

## Related Parameters
- [[motorcad/parameter_database/parameters/NumLoadPoints|NumLoadPoints]]
- [[motorcad/parameter_database/parameters/LoadPoint_Current_Array|LoadPoint_Current_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_PhaseAdvance_Array|LoadPoint_PhaseAdvance_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Torque_Array|LoadPoint_Torque_Array]]
- [[motorcad/parameter_database/parameters/LoadPoint_Calculated_Torque_Array|LoadPoint_Calculated_Torque_Array]]
- [[motorcad/parameter_database/parameters/HousingLoss_Hys_OC|HousingLoss_Hys_OC]]
- [[motorcad/parameter_database/parameters/HousingLoss_Hys_Adj_OC|HousingLoss_Hys_Adj_OC]]
- [[motorcad/parameter_database/parameters/HousingLoss_Fundamental_Hys_OC|HousingLoss_Fundamental_Hys_OC]]
- [[motorcad/parameter_database/parameters/HousingLoss_Minor_Hys_OC|HousingLoss_Minor_Hys_OC]]
- [[motorcad/parameter_database/parameters/HousingLoss_Total_Adj_OC|HousingLoss_Total_Adj_OC]]

## Related Wiki Pages
- [[equations/loss-equations|Loss Equations]]

## Source
Original CSV / Excel: `D:/SRM/Agent/workspace/wiki/raw/ActiveXParameters.xlsx`
