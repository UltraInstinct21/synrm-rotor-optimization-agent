...
Go to the [end](#page-11-0) to download the full example code.
...
**import math import ansys.motorcad.core as pymotorcad**
...
def write_motion_load_point_definition(mc, file, load_point_loop, num_duplications):
    ...
    write_name_value_pair(file, "symmetryMultiplier", str(num_duplications), 2, False,
    file.write(" }\n")
    file.write("}\n\n")
def get_electrical_periods_per_revolution(mc):
    # Handles SRM and BPM. Does not handle PMDC. For IM, this is per synchronous rotati
...
def get_mesh_node_id(position_id, num_nodes_stator, num_nodes_rotor, is_stator, is_main_no
...
def export_to_motion_unv(
    ...
    write_name_value_pair(
        unv_file, "MaxwellSimulationType", "2D, Element-based (Surface)", 1, True, Fal
    )
    unv_file.write("},\n\n")
    for i in range(num_load_points):
        write_motion_load_point_definition(mc, unv_file, i, num_duplications)
    unv_file.close()
    # Get the force data from Motor-CAD for stator and rotor, just to find number of no
    forces_stator_tangential_per_slice = mc.get_magnetic_3d_graph("Ft_Stator_OL_Lumped
    forces_rotor_tangential_per_slice = mc.get_magnetic_3d_graph("Ft_Rotor_OL_Lumped",
...
internal_node_id,
   ...
   # Node ID, followed by export coordinate system, displacement coordinate system
   # position X position Y position Z
   # Main node
   mesh_node_id = get_mesh_node_id(
       internal_node_id, num_nodes_stator, num_nodes_rotor, True, True
   )
   amesh_file.write(int_str_fixed_length_10(mesh_node_id) + " 1 1
   amesh_file.write(
...
# 9=Reaction Force, 2=Single precision float, 3=Number of values for the da
anf_file.write(" 1 4 2 9 2
...
anf_file.write(
   "- 0.00000e+00 0.00000e+00 0.00000e+00 0.00000e+00 0.00000e+00 0.0
)
...
for internal_node_id in range(num_nodes_stator):
   ...
   node_x, node_y, node_z, cos_angle, sin_angle = mechanical_node_positio
       internal_node_id,
       num_nodes_stator,
       stator_bore_radius,
       offset_angle_degrees,
   ...
   this_fx = (
       ...
       + forces_stator_tangential_per_slice.data[internal_node_id][step_i
       * sin_angle
   ) / slice_width
   this_fy = (
       -1
...
internal_node_id, num_nodes_stator, num_nodes_rotor, False, True
            ...
            this_fx = (
                ...
                - forces_rotor_tangential_per_slice.data[internal_node_id][step_ind
                * sin_angle
            ) / slice_width
            this_fy = (
                -1
...
# The offset_angle_degrees sets the position of the centre of a stator tooth relative
# If this is set to zero, the first stator tooth is aligned with the X axis.
offset_angle_degrees = 0
slice_centre_x = 0
slice_centre_y = 0
slice_centre_z = 0
# Connect to Motor-CAD. This assumes that the script is running from the scripting tab
...
<span id="page-11-0"></span>Created using [Sphinx](https://www.sphinx-doc.org/) 8.1.3.
