...
This example requires the use of a JSON configuration file. The ece\_config.json file should be saved to the same directory as this example Python script. You can download the ece\_config.json file from: [ansys/pymotorcad](https://github.com/ansys/pymotorcad/blob/main/examples/links/ece_config.json)
...
Define the read\_parameters function to import initial settings from a JSON file and return a dictionary:
...
Use the read\_parameters function to open the ece\_config.json configuration file and import the data as the in\_data dictionary.

The JSON configuration file must be saved to the same directory as this Python script. The ece\_config.json file can be downloaded from the PyMotorCAD GitHub repository: [ansys/pymotorcad](https://github.com/ansys/pymotorcad/blob/main/examples/links/ece_config.json)
...
The necessary data is extracted from the in\_data dictionary. The JSON configuration file contains:

- The Motor-CAD MOT filename to be used for the ECE export. If the file exists in the same directory as this Python script, it will be copied to the working\_folder location. If the file does not exist in the same directory as this Python script or the working\_folder location, the script opens the e8 Motor-CAD template by default.
- Operating parameters for the electric machine (shaft speed, DC bus voltage, temperature, maximum current, current resolution and number of points per cycle for the torque calculation). If using an input file, the file in the working\_folder will be modified by setting the operating parameter input settings.
...
Load the Motor-CAD file. If the mot\_file specified in the JSON configuration file exists in the same directory as this Python script, open the MOT file. If the file does not exist in the same directory as this Python script, check the working\_folder for the Motor-CAD file. The file will be modified by setting the operating parameter input settings and saved to the working\_folder .

If the file does not exist in the same directory as this Python script or the working\_folder , load the e8 IPM motor template and save the file to the working directory. Use the mot\_file filename that was taken from the JSON configuration file. Save input settings to a Motor-CAD MOT file.

```
if os.path.isfile(os.path.join(os.getcwd(), file_name)):
    shutil.copy(os.path.join(os.getcwd(), file_name), mot_file)
    print(f"Motor-CAD file copied from {os.path.join(os.getcwd(), file_name)} to {mot_
...
Opening Motor-CAD e8 template and saving to C:\Users\ansys\AppData\Local\Temp\twinbuild
...
By default, this script displays the figures and saves them to the results folder. Comment out the plt.show line if you do not want the figure to be displayed when the script is run.
...
plt.show() # comment out this line if you do not want the image to be displayed in a n
```
...
Calculate the alignment angle from the drive\_offset offset angle.
...
Import the saturation map data that was calculated and exported from Motor-CAD as the mat\_file\_data dictionary.

```
mat_file_data = io.loadmat(map_name)
```

Extract data from the mat\_file\_data dictionary:
...
For each input current combination and rotor position, the D and Q axis flux linkages, the homopolar component of the flux (approximated to zero) and the torque values are stored in the final\_table numpy array (look-up table).
...
final_table = np.array(
    [index_1, flux_d_2, flux_q_3, flux_0_4, torque_5, id_6, iq_7, phase_ad_8, rotor_po
)
```
...
for i in range(d_values):
    plt.plot(
        iq_peak[0, :], angular_flux_linkage_q[i, :, skip], label="Id=" + str(id_peak[i
    )
plt.ylabel("Flux [Vs]")
plt.xlabel("Iq [A]")
...
for i in range(d_values):
    plt.plot(
        iq_peak[0, :], angular_flux_linkage_d[:, i, skip], label="Id=" + str(id_peak[i
    )
plt.legend(fontsize=8, loc="lower right")
plt.ylabel("Flux [Vs]")
plt.xlabel("Iq [A]")
...
Generate the TXT file, using the path and filename that was taken from the ece\_config.json configuration file.
...
Generate the SML that will be loaded into Ansys Twin Builder to generate the ECE model. The SML file uses the phase resistance and end winding inductance and data from the look-up table. The SML file is saved using the path and filename taken from the ece\_config.json configuration file.
...
file_id.write(
   "INTERN II Fluxbd N1:=GND, N2:=N_10 ( QUANT:=AMFd.I, FACT:=cos(VM_Erad.V
   "\n"
)
file_id.write(
   "INTERN II Fluxbq N1:=GND, N2:=N_10 ( QUANT:=AMFq.I, FACT:=sin(VM_Erad.V
   "\n"
)
file_id.write("INTERN II Fluxbo N1:=GND, N2:=N_10" " ( QUANT:=AMFo.I, FACT:=
...
file_id.write(
   "INTERN II Fluxcd N1:=GND, N2:=N_15 ( QUANT:=AMFd.I, FACT:=cos(VM_Erad.V
   "\n"
)
file_id.write(
   "INTERN II Fluxcq N1:=GND, N2:=N_15 ( QUANT:=AMFq.I, FACT:=sin(VM_Erad.V
   "\n"
)
file_id.write("INTERN II Fluxco N1:=GND," " N2:=N_15 ( QUANT:=AMFo.I, FACT:=
...
file_id.write(
   "INTERN II Id0 N1:=GND, N2:=N_19 ( QUANT:=AMa0.I, FACT:=2/3*cos(VM_Erad
)
file_id.write(
   "INTERN II Id1 N1:=GND, N2:=N_19 ( QUANT:=AMb0.I, FACT:=2/3*cos(VM_Erad
   "\n"
)
file_id.write(
   "INTERN II Id2 N1:=GND, N2:=N_19 ( QUANT:=AMc0.I, FACT:=2/3*cos(VM_Erad
   "\n"
)
file_id.write("INTERN AM AM0 N1:=N_19," " N2:=GND ;\n")
file_id.write(
   "INTERN II Iq0 N1:=GND, N2:=N_20" " ( QUANT:=AMa0.I, FACT:=2/3*sin(VM_E
)
file_id.write(
   "INTERN II Iq1 N1:=GND, N2:=N_20 ( QUANT:=AMb0.I, FACT:=2/3*sin(VM_Erad
   "\n"
)
file_id.write(
   "INTERN II Iq2 N1:=GND, N2:=N_20 ( QUANT:=AMc0.I, FACT:=2/3*sin(VM_Erad
   "\n"
)
...
file_id.write(
   "INTERN IV " "Gx N1:=GND," " N2:=N_24 ( QUANT:=VM_Speed.V, FACT:=57.2957
)
file_id.write("INTERN C" " " "Cx N1:=N_24, N2:=GND ( C:=1, V0:=IniPos ); \
file_id.write("INTERN VM" "" " VM_Mdeg N1:=N_24, N2:=GND ; \n")
file_id.write("INTERN IV" "" " Ipos N1:=GND, N2:=N_25 ( QUANT:=VM_Mdeg.V, FA
