...
A MotorCADTwinModel class has been created to encapsulate the required logic. The resulting object contains the data and functions to export the required Motor-CAD in the appropriate format. A summary of the operations performed is as follows:
...
- 3. The cooling system nodes and flow path are identified and saved to the CoolingSystems.csv file
- 4. For each desired speed, the thermal model is solved and thermal matrices exported and saved to the dpxxxxxx folders
- 5. The distribution of the losses onto the individual nodes is determined and saved to the LossDistribution.csv file
- 6. Natural convection cooling of the Housing is characterized and saved to the HousingTempDependency folder
- 7. Temperature dependent Airgap heat transfer is characterized and saved to the AirGapTempDependency folder

```
class MotorCADTwinModel:
    # Store required constants for the Motor-CAD Cooling System Node Group names (provi
    # ``.nmf`` file), corresponding parameter names for varying flowrate and inlet temp
    # the Motor-CAD loss names (for display in Twinbuilder), and the corresponding Moto
    # parameter names.
    coolingSystemData = {
        "End Space": [],
        "Ventilated": ["TVent_Flow_Rate", "TVent_Inlet_Temperature"],
        "Housing Water Jacket": ["WJ_Fluid_Volume_Flow_Rate", "HousingWJ_Inlet_Temperat
        "Shaft Spiral Groove": [
        ...
        "Spray Cooling": ["Spray_Cooling_Fluid_Volume_Flow_Rate", "Spray_Cooling_Inlet_
        "Rotor Water Jacket": ["Rotor_WJ_Fluid_Volume_Flow_Rate", "RotorWJ_Inlet_Temp"
        "Slot Water Jacket": ["Slot_WJ_Fluid_Volume_Flow_Rate", "Slot_WJ_Fluid_inlet_te
        "Heat Exchanger": [],
    }
    lossNames = [
        "Armature_Copper_dc",
...
# Stator_Iron_Loss_@Ref_Speed_[Tooth_Tip] is not included as this is an unused Moto
# parameter
...
def __init__(self, inputMotFilePath: str, outputDir: str):
    ...
    # check which Motor-CAD version is being used as this affects the resistance ma
    self.motorcadV2025OrNewer = self.mcad.connection.check_version_at_least("2025.
    self.mcad.load_from_file(self.inputMotFilePath)
# Main function to call which generates the required data for the Twin Builder expo
def generateTwinData(
    ...
    self.updateMotfile()
    # calculate self.nodeNames, self.nodeNumbers, self.nodeGroupings, self.nodeNumb
...
# Helper functions to parse the exported Motor-CAD matrices (``.cmf``, ``.nmf``, `
# ``.rmf`` and .``.tmf``)
...
def getRmfData(self, exportDirectory):
    ...
    # resistance matrix exported by v2025R1 and newer is transposed vs older versio
    if self.motorcadV2025OrNewer:
...
# Functions to set and get the losses in the model, used to ensure the calculations
...
def updateMotfile(self):
    ...
    ## turn off any temperature scaling losses as will affect loss distribution ca
```
...
self.mcad.set_variable("StatorCopperLossesVaryWithTemp", 0)
    ...
    # save the updated model so it is clear which Motor-CAD file can be used to va
...
# Helper function that solves the Motor-CAD thermal network and exports the matrice
...
def getNodeData(self):
    ...
    self.nodeNumbers, self.nodeNames, self.nodeGroupings = self.getNmfData(exportD
...
# Function that determines the nodes used for the cooling system and their connecti
...
```
self.nodeNames[self.nodeNumbers.index(grap
                                    ),
                                ]
                            )
            ...
            plt.savefig(os.path.join(self.outputDirectory, str(graphNode) + "_cool
        # write cooling systems config file
        if len(connectedNodesLists) > 0:
            with open(os.path.join(self.outputDirectory, "CoolingSystems.csv"), "w
                k = 0
...
# Function that runs the thermal model at each desired speed, and exports the therm
def generateSamples(self, parameters: dict):
    ...
    for index, rpm in enumerate(rpmSamples):
        ...
        exportDirectory = os.path.join(self.outputDirectory, "dp" + str(index).zfi
...
parameterValues = list(parameters.values())
        ...
        for i in range(0, nbDPs):
            ...
            cf.write("\n")
# Function that extracts the per-node loss distribution for each loss type, allowin
# specify a loss value using a name (such as Armature Copper Loss) and have Twin Bu
...
def generateLossDistribution(self):
    ...
    for lossIndex in range(numLossParameters):
        ...
        exportDirectory = os.path.join(self.outputDirectory, "tmp", "dis" + str(lo
        ...
        for nodeIndex, nodePower in enumerate(powerVector):
            ...
            if nodePower > 0:
                lossDistributionMatrix[lossIndex, nodeIndex] = nodePower / inputLo
    with open(os.path.join(self.outputDirectory, "LossDistribution.csv"), "w") as
        outfile.write(" ")
        ...
        for index, lossName in enumerate(self.lossNames):
            ...
            outfile.write("\n")
# Function that determines the Housing to Ambient resistances at different housing
# the results of which are used by Twin Builder to take into account external Natu
# cooling.
...
```
    ...
    fileInd = 0
    for ambientTemperature, fixedHousingTemperatures in housingAmbientTemperatures
        fileInd = fileInd + 1
        ...
        with open(
            os.path.join(exportDirectory, "Housing_Temp" + str(fileInd) + ".csv"),
        ) as fout:
...
def computeMatricesHousingTemps(
    ...
    for housingTemperature in fixedHousingTemperatures:
        ...
        for housingNode in housingNodes:
            ...
            self.mcad.set_fixed_temperature_value(name, housingNode, housingTemper
        self.computeMatrices(exportDirectory)
        resistanceMatrix = self.getRmfData(exportDirectory)
...
# Function that determines the Stator to Rotor airgap resistance at different housi
# temperatures, the results of which are used by Twin Builder to take into account
```
...
def generateAirgapTempDependency(self, rpmSamples, airgapTemperatures):
    ...
    for rpm in rpmSamples:
        ...
        with open(os.path.join(exportPath, "AirGap_Temp" + str(fileInd) + ".csv"),
            fout.write(str(rpm) + "\n")
            for key, item in file_content.items():
...
def computeMatricesAirgapTemp(self, airgapTemperatures, rpm):
    ...
    for airgapTemperature in airgapTemperatures:
        ...
        self.mcad.set_fixed_temperature_value(name, airgapNodeStator, airgapTemper
        name = "Airgap_Rotor_Node_" + str(airgapNodeRotor)
        self.mcad.set_fixed_temperature_value(name, airgapNodeRotor, airgapTempera
        self.computeMatrices(exportDirectory, rpm=rpm)
        resistanceMatrix = self.getRmfData(exportDirectory)
...
def getAirgapNodes(self):
    ...
    if tVent or sVent:
        ...
        if statorCoolingOnly == False:
            warnings.warn(
                "Temperature dependent airgap not supported for ventilated cooling
                "flow"
            )
...
```
warnings.warn("Temperature dependent airgap only supported for middle axia
...
def generateCoolingSystemsInputsDependency(self, coolingSystemsInputs):
    for index, (coolingSystem, parameters) in enumerate(coolingSystemsInputs.items
        if coolingSystem not in self.coolingSystemData:
            warnings.warn(
                "The Cooling System name {} is not part of the list of Cooling Syst
                "{}\n".format(coolingSystem, self.coolingSystemData)
            )
            return
        exportPath = os.path.join(self.outputDirectory, self.unbracket(coolingSyst
        if not os.path.isdir(exportPath):
            os.makedirs(os.path.join(exportPath))
        rpms = parameters["rpm"]
        frs = parameters["FR"]
        ...
        with open(os.path.join(exportPath, "c_nodes.txt"), "w") as fCout, open(
            ...
            for inNode, conList in self.coolingSystems.items():
                if self.nodeGroupings[self.nodeNumbers.index(inNode)] == coolingSy
                    upnode = inNode
                    coolSys = conList
```
...
for i in range(0, len(connectedNodes)):
    fRout.write(
        ...
        + " "
        + self.unbracket(self.nodeNames[self.nodeNumbers.index(connect
        + "\n"
    )
    r_list.append(
        [
...
for item in coolSys: # following downstream nodes of the cooling syste
    for upnode in item:
        if upnode not in list(covered_nodes.keys()):
            ...
            for i in range(0, len(connectedNodes)):
                ...
                ): # avoid taking the symmetric counterpart of the res
                    fRout.write(
                        self.unbracket(
                            self.nodeNames[self.nodeNumbers.index(upnod
                        )
                        + " "
                        + self.unbracket(
                            self.nodeNames[
                                self.nodeNumbers.index(connectedNodes[
                            ]
                        )
                        + "\n"
                    )
            ...
            if upnode not in self.nodeNumbers_fluidInlet:
                fCout.write(
                    self.unbracket(self.nodeNames[self.nodeNumbers.inde
                    + "\n"
                )
                c_list.append([self.nodeNames[self.nodeNumbers.index(u
            covered_nodes.update({upnode: connectedNodes})
if (
    len(coolSys) == 0
): # particular case where the cooling system has only 2 nodes (inlet/
    for upnode in connectedNodes:
```

```
if (
                self.nodeGroupings[self.nodeNumbers.index(upnode)] == cool
            ): # make sure the connected node still belongs to cooling sys
                connectedNodes = self.returnConnectedNodes(
                    upnode, self.nodeNumbers, resistanceMatrix
                )
                for i in range(0, len(connectedNodes)):
                    ...
                    if upnode not in self.nodeNumbers_fluidInlet:
                        ...
                        c_list.append([self.nodeNames[self.nodeNumbers.inde
                covered_nodes.update({upnode: connectedNodes})
# run the DoE
fileInd = 0
...
def computeMatricesCoolingSystemsInput(
    ...
    exportDirectory = os.path.join(self.outputDirectory, "tmp", "dp" + str(fileInd
    if not os.path.isdir(exportDirectory):
        os.makedirs(exportDirectory)
    self.mcad.set_variable("Shaft_Speed_[RPM]", rpm)
...
Below is an example of how the above MotorCADTwinModel class can be used using the e8\_eMobility template .mot file.
...
{20: [20, 20.5, 21.0, 21.5, 22.0, 22.5, 23.0, 23.5, 24.0, 24.5, 25.0, 26.0, 27.0, 28.0,
```
...
Motor-CAD input file: C:\actions\_runner\_docs\\_work\pymotorcad\pymotorcad\examples\links TB data output dir: C:\actions\_runner\_docs\\_work\pymotorcad\pymotorcad\examples\links\t
