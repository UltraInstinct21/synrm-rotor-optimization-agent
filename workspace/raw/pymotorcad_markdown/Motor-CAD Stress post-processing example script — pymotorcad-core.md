...
# Motor-CAD Stress postprocessing example script
...
class Element:
    ...
    strain_plastic_glinka : float
        The plastic strain using the Glinka correction. Populated by apply_glinka_corre
    """
    def __init__(
        self, tri_index, node_1, node_2, node_3, x, y, s_x, s_y, t_xy, sp_1, sp_2, svm
    ):
...
def apply_neuber_correction(self, youngs_modulus, non_linear_strain, non_linear_stress):
    ...
    non_linear_strain, non_linear_stress : np.ndarray
        Corresponding arrays of strain (ratio) and stress (MPa) for the non-linear
    """
    ...
    # If on the elastic portion, just return inputs, so we don't get interpolation
    ...
    if elastic_stress_strain_product > np.max(non_linear_stress_strain_product):
        raise ValueError(
            "Input too large (elastic stress strain product > maximum stress strai
            "non-linear data). "
            "Elastic stress is "
            + str(elastic_stress)
    ...
    equivalent_non_linear_strain = np.interp(
        elastic_stress_strain_product, non_linear_stress_strain_product, non_linea
    )
...
def apply_glinka_correction(self, youngs_modulus, non_linear_strain, non_linear_stress):
    ...
    equivalent_non_linear_strain = np.interp(
        elastic_stress_strain_integral, non_linear_stress_strain_integral, non_line
    )
    # Lookup to find non-linear stress at this strain:
```
...
The Von Mises stress in MPa
    ...
    self.elements.append(
        Element(
            tri_index, node_1, node_2, node_3, x, y, s_x, s_y, t_xy, sp_1, sp_2, s
        )
    )
...
result_array = []
    ...
    def apply_corrections(self, non_linear_strain, non_linear_stress):
        """Update the Neuber and Glinka estimates using the Von Mises stress for all e
        Parameters
        ----------
        non_linear_strain, non_linear_stress : np.ndarray
            Corresponding arrays of strain (ratio) and stress (MPa) for the non-linear
        ...
        # In principle, we could not apply a scaling based on the corrected stresses to
        # s_x, s_y and t_xy stresses, so we have an offset to apply to account for rela
...
def get_stress_data(mc, clean_up=True):
    ...
    This function should be called after the stress calculation has been carried out us
    ...
    temp_filename = pathlib.Path(tempfile.gettempdir()) / pathlib.Path(str(uuid.uuid4(
    # The Motor-CAD FEA file always saves with '.' as a decimal separator, independent
    ...
    # Read first line, second value will be length of element table (e.g. '1 41230 Elem
...
Load a template (for users this would normally be replaced by load\_from\_file(), and disable popup messages.
...
Apply the Glinka and Neuber stress corrections to the regions of interest using the region's apply\_corrections method, and plot the results.
